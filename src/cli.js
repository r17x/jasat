...
const run = () => {
  ...
  const noOp = () => {}
  const hash = (fileBuffer)  => crypto.createHash('sha256').update(fileBuffer).digest('hex');
  const isModifiedContent = (a, b) => hash(a) !== hash(b)
  const pathToBasename = p => path.parse(p).name
  const getTransforms = async () => await glob(path.join(__dirname, "transforms", "*.js"))

  const showList = () => getTransforms()
    .then(files => process.stdout.write(["", ...files, ""].map(files => pathToBasename(files)).join("\n")))
  ...
  const runTransform = async () => {
    ...
    const transform = await getTransforms()
      .then(xs => xs.find(transformFile => pathToBasename(transformFile) === transformName))
   
    return ({
      [true]: noOp,
      [Boolean(transform)]: async () => {
        const cwd = flags.cwd || process.cwd()
        const targetFiles = await glob(path.join(cwd, TARGET_PATTERN))
          .then(sources => sources.map(async source => {
        ...
        return Promise.all(targetFiles)
          .then(files => {
            if(flags.write){
              files.forEach (
                file => file.modified ? fs.writeFile(file.location, file.newContent, "utf8") : noOp()
              )
            }
            return files
          })
...
${chalk.yellow(flags.write ? "Modified:" : "Found:")} ${chalk.bold.yellow(modified)}`
          )}
        )
      }
    }).true()
